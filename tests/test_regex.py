import lollipop_hypothesis.regex as r
import pytest
import re
import six.moves


def assert_can_generate(regex):
    strategy = r.strategy(regex)
    for _ in six.moves.range(100):
        s = strategy.example()
        assert re.match(regex, s) is not None, \
            '"%s" supposed to match "%s" (strategy = %s)' % (s, regex, strategy)
